...
const { onlineUsers } = require('../socketServer');
...
async function listMatches(req, res) {
  ...
  try {
    ...
    const matches = result.rows.map(row => ({
      ...row,
      is_online: onlineUsers ? onlineUsers.has(Number(row.other_user_id)) : false
    }));

    res.json(matches);
  } catch (err) {
    console.error('Error listing matches:', err);
    res.status(500).json({ error: 'Server error listing matches' });
  }
}
